...
const ALARM_NAME = 'autoSaveAlarm';
  ...
  chrome.storage?.onChanged?.addListener(async (changes, areaName) => {
    try {
      ...
      if (changes?.tabSaverSettings) {
        ...
        if (newSettings) {
          await setupAutoSave(newSettings);
        }
      }
    } catch (error) {
...
async function setupAutoSave(settings) {
  ...
  if (settings.autoSave && settings.autoSaveInterval > 0) {
    ...
    console.log(`Auto-save enabled: every ${settings.autoSaveInterval} minutes`);
  } else {
...
async function autoSaveTabs() {
  try {
    ...
    // Limit auto-saves to prevent storage bloat (keep last 5 auto-saves)
    ...
    // await showNotification('Auto-save complete', `Saved ${tabData.length} tabs`);
...
async function handleMessage(request, sender) {
  switch (request.action) {
    ...
    case 'settingsUpdated':
      await setupAutoSave(request.settings);
      return { success: true };
